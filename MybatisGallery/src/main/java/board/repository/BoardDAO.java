...
public class BoardDAO {
	...
	public int insert(Board board) {
		...
		//sqlSession.insert("쿼리문 들어있는 xml 코드조각 id", sqlSession);
	...
	public Board select(int board_idx) {
		Board board = null;
		SqlSession sqlSession = config.getSqlSession();
		board =sqlSession.selectOne("Board.select", board_idx);		//selectOne = 하나만 가져 오겠다
		config.release(sqlSession);
		return board;
	}
	...
	public int update(Board board) {
		int result =0;
		SqlSession sqlSession = config.getSqlSession();		//얻어오기

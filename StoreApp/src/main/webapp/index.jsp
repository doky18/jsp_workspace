...
function createOption(categoryList){
	...
	for(let i=0; i<categoryList.length; i++){
		let category=categoryList[i];		//
		 ...
		 op+=category.category_name;
	     op+="</option>";
	}
	
	$("#category_idx").html(op);		//inner HTML
}
...
function createMarker(map, storeList){
	for(let i=0; i<storeList.length; i++){//맛집 수 만큼 마커를 생성하자 json 배열..
...
//post 전송 + 비동기
//text : content-type :applicaton/x-www-form-urlencoded
//binary : content-type : multipart/
function regist(){
	let xhttp= new XMLHttpRequest();
